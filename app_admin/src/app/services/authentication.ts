...
export class AuthenticationService {
  ...
  public getToken(): string {
    ...
    // Make sure we return a string even if we dont have a token
    if(!out) 
    {
      return '';
    }
  ...
  // still valid. Even if we have a token we still have to
  // reauthenticate if our token has expired
  ...
  // login method becasue the behavior of the API logs a new user in
